...
class Matrix implements \IteratorAggregate, \ArrayAccess, \Countable
{
    ...
    public static function from($dataSource)
    {
        if (empty($dataSource)){
            $dataSource = [];
        } elseif (!(is_array($dataSource) || is_string($dataSource) || ($dataSource instanceof \ArrayAccess))){
            if ($dataSource instanceof \Traversable){
                ...
                throw new \InvalidArgumentException('Invalid $dataSource! It should be an array, string, kinda ArrayAccess or at least kinda Traversable.');
            }
        }
    ...
    public function select($keyNames)
    {
        ...
        foreach ($this->data as $rowData) {
            ...
            foreach ($keyAliasMap as $key => $alias) {
                $selectedItem[$alias] = $this->fetchCellFromRow($rowData, $key);
            }
            ...
            case self::CONDITION_SIMPLE_MATCH:
                foreach ($this->data as $rowData) {
                    ...
                    foreach ($conditions as $key => $conditionValue) {
                        $cellValue = $this->fetchCellFromRow($rowData, $key);
    ...
    public function indexedBy($columnName, $keyNameGlue = "_")
    {
        ...
        if (!is_array($columnName)) {
            ...
            foreach ($this->data as $rowData) {
                $keyValue = $this->fetchCellFromRow($rowData, $columnName);
                $indexedData[$keyValue] = $rowData;
            }
        } else {
            ...
            foreach ($this->data as $rowData) {
                ...
                foreach ($columnName as $key) {
                    $keys[] = $this->fetchCellFromRow($rowData, $key);
                }
    ...
    public function groupBy($columnName, $groupKeyGlue = "_")
    {
        ...
        if (!is_array($columnName)) {
            // 将数据转换为以$keyName那一列为key的数组
            foreach ($this->data as $rowData) {
                $groupKey = $this->fetchCellFromRow($rowData, $columnName);
     ...
     *
     * @param mixed         $needle
     * @param callable|null $equals 查找函数(参数分别是$needle和$elem, 返回true/false分别代表找到/未找到)
     * @return int 成功则返回对应的索引，失败(找不到)则返回-1
     */
    public function indexOf($needle, $equals = null)
    {
        if (empty($equals)) {
            $index = array_search($needle, $this->toArray());
        } else if (is_callable($equals)) {
            foreach ($this->data as $index => $elem) {
                if (call_user_func($equals, $needle, $elem)) {
                    return $index;
                }
            }
            return -1;
        } else {
            throw new \InvalidArgumentException('$equals must be empty or a callable!');
        }
     ...
     *                             'name' => array(SORT_ASC),
     *                             'age' => SORT_NUMERIC,
     *                             // 按照某个列表进行排序，此时要传入一个Matrix对象
     *                             // 不在列表中的值排在最后
     *                             'gender' => Matrix::from(['male', 'female'])
     *                   )
        ...
        if (is_array($keyNames)) {
            ...
            foreach ($keyNames as $key => $option) {
                ...
                if (is_int($key)) {
                    ...
                    $args[] = $orderType;
                    $args[] = $sortFlag;
                } else if ($option instanceof Matrix) { // 按照列表进行排序
                    $args[] = $this->duplicate()->map(function ($rowData) use ($key, $option) {
                        $cell = $this->fetchCellFromRow($rowData, $key);
                        $index = $option->indexOf($cell);
                    ...
                    if (is_array($option)) {
                        if (count($option) > 2){
                            throw new \InvalidArgumentException('Invalid sort option! It must be a SORT_XXX flag or a array of at most 2 SORT_XXX flags!');
                        }

                        if (in_array($option[0], array(SORT_ASC, SORT_DESC))) {
                            $args[] = $option[0];
                            $args[] = $option[1];
                        } else {
                            $args[] = $option[1];
    ...
    public function firstColumn()
    {
        ...
        $data = array_map(function ($rowData) {
            return $this->fetchFirstCellInRow($rowData);
        }, $data);
        return $data;
    }
    ...
    public function column($columnName)
    {
        ...
        $data = array_map(function ($rowData) use ($columnName) {
            return $this->fetchCellFromRow($rowData, $columnName);
        }, $data);
        return $data;
    }
    ...
    public function toArray($associated = true)
    {
        if (is_array($this->data)) {
            if ($associated) {
                ...
                return array_map(function ($rowData) {
                    return Matrix::from($rowData)->values();
                }, $this->data);
            }
        }
        ...
        if ($associated) {
            ...
            foreach ($this->data as $rowData) {
                $arr[] = Matrix::from($rowData)->values();
            }
        }
     ...
     * @return mixed
     */
    public function fetchCell($rowIndex, $columnIndex)
    {
        $rowData = $this->fetchCellFromRow($this->data, $rowIndex);
        return $this->fetchCellFromRow($rowData, $columnIndex);
    }
     ...
     * @return mixed
     */
    public function fetchCellFromRow($rowData, $column)
    {
        if ($column instanceof \Closure) {
            return call_user_func($column, $rowData);
        } else if (is_array($rowData) || (is_object($column) && $rowData instanceof \ArrayAccess)) {
     ...
     * @return mixed|null
     */
    public function fetchFirstCellInRow($rowData)
    {
        if (is_array($rowData)) {
            return reset($rowData);
        } else if (is_object($rowData)) {
            if ($rowData instanceof \Traversable) {
                foreach ($rowData as $cell) {
                    return $cell;
                }
                return null;
            } else if ($rowData instanceof \ArrayAccess) {
                return $rowData[0];
            } else {
                $first = '0';
